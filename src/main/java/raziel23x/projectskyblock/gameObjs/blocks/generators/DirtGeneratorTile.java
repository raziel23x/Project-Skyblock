...
public class DirtGeneratorTile extends TileEntity implements ITickableTileEntity {
    protected ItemStackHandler handler = new ItemStackHandler() {
        @Override
        public boolean isItemValid(int slot, @Nonnull ItemStack stack) {return stack.getItem() == Items.DIRT;}
    };
    ...
    public void tick() {
        if (!world.isRemote) {
            ...
            if (ticks == 10) {
                ...
                ItemHandlerHelper.insertItemStacked(handler, new ItemStack(Items.DIRT, 1), false);
                ...
                if (tile != null && tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, Direction.DOWN).isPresent()) {
                    ...
                    if (handler.getStackInSlot(0) != ItemStack.EMPTY) {
                        ItemStack stack = ItemHandlerHelper.insertItem(outHandler, new ItemStack(Items.DIRT, 1), true);
