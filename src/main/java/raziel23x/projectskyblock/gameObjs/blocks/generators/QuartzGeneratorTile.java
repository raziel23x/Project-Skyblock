...
import static raziel23x.projectskyblock.init.ModEntityType.QUARTZGENERATOR_TILE;
...
public class QuartzGeneratorTile extends TileEntity implements ITickableTileEntity {
    ...
    public QuartzGeneratorTile() {
        super(QUARTZGENERATOR_TILE.get());
    }
    ...
    public void tick() {
        if (!world.isRemote) {
            ...
            if (ticks == 10) {
                ...
                ItemHandlerHelper.insertItemStacked(handler, new ItemStack(Items.QUARTZ, 1), false);
                ...
                if (tile != null && tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, Direction.DOWN).isPresent()) {
                    ...
                    if (handler.getStackInSlot(0) != ItemStack.EMPTY) {
                        ItemStack stack = ItemHandlerHelper.insertItem(outHandler, new ItemStack(Items.QUARTZ, 1), true);
