...
public class SoulsandGeneratorBlock extends Block {
    private static final DirectionProperty FACING = HorizontalBlock.HORIZONTAL_FACING;

    public SoulsandGeneratorBlock() {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new SoulsandGeneratorTile();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult rayTraceResult) {
        ...
        if (tileEntity != null) {
            if (heldItem.isEmpty() || heldItem.getItem() == Items.SOUL_SAND) {
                Objects.requireNonNull(worldIn.getTileEntity(pos)).getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
