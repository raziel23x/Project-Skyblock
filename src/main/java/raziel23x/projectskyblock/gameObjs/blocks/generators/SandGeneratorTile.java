...
import static raziel23x.projectskyblock.init.ModEntityType.SANDGENERATOR_TILE;
...
public class SandGeneratorTile extends TileEntity implements ITickableTileEntity {
    protected ItemStackHandler handler = new ItemStackHandler() {
        ...
        {
            return stack.getItem() == Items.SAND;
        }
    };

    private int ticks;
    public SandGeneratorTile() {
        super(SANDGENERATOR_TILE.get());
    }
    ...
    public void tick() {
        if (!world.isRemote) {
            ...
            if (ticks == 10) {
                ...
                ItemHandlerHelper.insertItemStacked(handler, new ItemStack(Items.SAND, 1), false);
                ...
                if (tile != null && tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, Direction.DOWN).isPresent()) {
                    ...
                    if (handler.getStackInSlot(0) != ItemStack.EMPTY) {
                        ItemStack stack = ItemHandlerHelper.insertItem(outHandler, new ItemStack(Items.SAND, 1), true);
