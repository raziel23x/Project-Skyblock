...
import static raziel23x.projectskyblock.init.ModEntityType.NETHEREACKGENERATOR_TILE;

public class NetherrackGeneratorTile extends TileEntity implements ITickableTileEntity {
    protected ItemStackHandler handler = new ItemStackHandler() {
        @Override
        public boolean isItemValid(int slot, @Nonnull ItemStack stack) {return stack.getItem() == Items.NETHERRACK;}
    };

    private int ticks;
    public NetherrackGeneratorTile() {
        super(NETHEREACKGENERATOR_TILE.get());
    }
    ...
    public void tick() {
        if (!world.isRemote) {
            ...
            if (ticks == 10) {
                ...
                ItemHandlerHelper.insertItemStacked(handler, new ItemStack(Items.NETHERRACK, 1), false);
                ...
                if (tile != null && tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, Direction.DOWN).isPresent()) {
                    ...
                    if (handler.getStackInSlot(0) != ItemStack.EMPTY) {
                        ItemStack stack = ItemHandlerHelper.insertItem(outHandler, new ItemStack(Items.NETHERRACK, 1), true);
