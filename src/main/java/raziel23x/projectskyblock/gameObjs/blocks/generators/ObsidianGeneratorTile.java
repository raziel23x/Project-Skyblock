...
import static raziel23x.projectskyblock.init.ModEntityType.OBSIDIANGENERATOR_TILE;
...
public class ObsidianGeneratorTile extends TileEntity implements ITickableTileEntity {
    ...
    public ObsidianGeneratorTile() {
        super(OBSIDIANGENERATOR_TILE.get());
    }
    ...
    public void tick() {
        if (!world.isRemote) {
            ...
            if (ticks == 10) {
                ...
                ItemHandlerHelper.insertItemStacked(handler, new ItemStack(Items.OBSIDIAN, 1), false);
                ...
                if (tile != null && tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, Direction.DOWN).isPresent()) {
                    ...
                    if (handler.getStackInSlot(0) != ItemStack.EMPTY) {
                        ItemStack stack = ItemHandlerHelper.insertItem(outHandler, new ItemStack(Items.OBSIDIAN, 1), true);
